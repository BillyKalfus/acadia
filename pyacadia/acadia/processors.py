...
class Processor(ABC):
    ...
    def instruction(cls, name=None):
        ...
        def named_instruction_decorator(compilation_func):
            # We will intentionally return None from this decorator since we
            # don't want to actually create a function for the class, we just
            # want to cache it in the instruction set
    ...
    def __init__(self, instruction_limit=None):
        """
        A base class for objects that represent entities capable of being 
        commanded by a set of native "instructions". Native instructions are
        defined in subclasses by decorating methods that produce machine code 
        with :meth:`Processor.instruction` (see its documentation for a 
        description of how instructions are implemented). 
        
        Instances of :class:`Processor` maintain an internal list of all 
        instructions invoked on it, referred to as the "program list". The 
        order of the instructions inside the program list is understood to 
        be the order in which the instructions will execute, in the absence 
        of branching. 
        ...
        load the instructions following the branch). Here, we explicitly expose
        this decision to the user with the keyword `inline`.
        
        Compiling a program for a :class:`Processor` consists of three distinct 
        steps:
        ...
        #. Resource Allocation
            ...
            :class:`Processor` objects in a script, this creates 
            :class:`Instruction` objects which are then stored in the program 
        ...
        for instruction_name,translator in self.__class__._instruction_set.items():
            def append_instruction(proc_self, *args, **kwargs):
                ...
                * `instruction`: The name of the instruction to execute, which 
                   is used to look up the translation function in the 
                   dictionary defining the class' instruction set.
                ...
                * `block_end`: If `True`, indicates that this instruction is 
                   the lats in a non-inlined block
                   
                * `cache`: A list of objects cached during compilation. This
                field is automatically populated during compilation and should
                ...
                * `block_level`: The nesting level of the block to which this
                ...
                """
                instruction_resource = proc_self.Instruction({
                    "instruction": instruction_name, 
                    "args": args, 
                    "kwargs": kwargs, 
                    "cache": [], 
                    "block_start": proc_self.block_start, 
                    "block_end": proc_self.block_end})
                
                proc_self.block_start = False
                proc_self.block_end = False
                
                if precompile:
                    proc_self.compile_instruction(instruction_resource)
    ...
    def block_end(self):
        """
        Indicate that the next instruction called is the last in a non-inlined 
        block.
        """
        self.block_end = True
    ...
    def compile_all(self):
        ...
        calling :meth:`comple_single` on every instruction, while restructuring 
        the program to obey the desired block structure.
        """
        blocks = [[]]
        block_prev = None
        block_current = 0
        ...
        for instruction in self.Instruction.instances:
            ...
            blocks[block_current].extend(compiled_instructions)
...
class ProcessorControlFlowMixin(ABC):
    """
    A mixin for subclasses of :class:`Processor` with capabilities
    for testing Boolean expressions (:meth:`test`), looping over
    iterables (:meth:`loop`), and waiting for a Boolean expression to be 
    satisfied (:meth:`await`). It's worth noting that these are direct 
    analogues of typically-extant `if`, `for`, and `while` loops, but given
    the potential for specialized behavior on different hardware, we rename
    these control structures to avoid confusion.
    """
...
class PythonProcessor(Processor, ProcessorSubroutineMixin):
    ...
    def __init__(self, instruction_limit=None):
        ...
        # Create a cache for runtime Python variables
        self.data = {}
    ...
    def __call__(self, instruction_resource):
        """
        Compile the execution of a Python statement.
        """
        ...
        if isinstance(op, str):
            ...
            compiled_op = self.compile_arg(op, instruction_resource)
    ...
    def call_subroutine(self, instruction_resource):
        ...
        code_string = f"self._subroutines[{instruction_resource['instruction']}](*({instruction_instance}[\'args\']), **({instruction_instance}[\'kwargs\']))"
        return [code_string]
