...
def run(plot=True):
    ...
    qubit_frequencies = np.linspace(3.9e9, 4.1e9, 201)
    readout_frequencies = np.linspace(9.2153e9-0.5e6, 9.2153e9+0.5e6, 11)

    # Run the program on the target
    rt = QubitSpectroscopyRuntime(
        qubit_frequencies=qubit_frequencies,
        readout_frequencies=readout_frequencies,
        qubit_stimulus=qubit_stimulus,
        readout_stimulus=readout_stimulus,
        readout_capture=readout_capture,
        iterations=1000,
