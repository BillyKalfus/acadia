...
\subsubsection{Direct-to-memory streaming}

\subsubsection{Complex multiplier with accumulator (CMACC)}


The Complex Multiplier and Accumulator (CMACC) module provides a low-latency solution for filtering and decimating signals in real time. Each sample that enters the CMACC is multiplied with the corresponding sample of an arbitrary complex kernel/window function, and the result is iteratively summed (``accumulated'') in a register (the ``accumulator''). By choosing an appropriate window function, different bands of noise can be rejected from the incoming signal. This architecture can also allow the signal to be processed with a matched filter, which optimizes the noise rejection when trying to distinguish between known signals.
...
\begin{enumerate}
  \item
  \begin{itemize}
    \item Four complex samples enter the CMACC (each comprised of two 16-bit signed quadratures) which are summed together to produce a complex value with 18-bit signed quadratures.
...
An internal pointer register is used to step through the kernel memory as new samples enter the CMACC so that the samples can be multiplied by the appropriate value of the kernel. The pointer is initialized to the pre-programmed start address via a register write and it increments for every valid sample that enters the CMACC. When it reaches the pre-programmed end address, it resets to the starting address. If valid samples continue to enter the CMACC, the pointer will continue to increment again without any additional latency or dropped data; this enables the use of periodic kernels without needing to occupy more than a single period's worth of kernel memory. A common special case is the ``boxcar'' kernel, which only requires loading a single entry in kernel memory. This feature can also be used for decimation, since the CMACC can be configured to emit a valid output upon reaching the kernel end address (see below). The sequencer can also manually reset the kernel pointer to its start address via a write to the configuration register, which is required after updating the kernel start and end addresses.

The CMACC does not require consecutive samples to be valid. When samples enter the CMACC, if they are marked as not valid, they will not contribute to the accumulator and the kernel pointer will not increase. If an input sample is marked as ``last'', the CMACC can be configured to reset the accumulator and/or produce an output; see below for a description of accumulator configuration and output behavior.
