...
input_valid = np.empty((544-412) // 4, dtype=bool)
input_valid.fill(True)
input_valid[(484-412) // 4] = False
...
for i in range(len(input_valid)):
    ...
    print(f"-------- Input cycle {i} (sim time = {412 + i*4}) --------")
    ...
    print(f"Summed input data: evaluate just after transition at t={412 + i*4 + 4}")
    print(f"({a_re:08x}, {a_im:08x})")
    print("")

    print(f"Partial products and full product: evaluate just after transition at t={412 + i*4 + 8}")
    ...
    # print(f"Starting accumulator value: evaluate just before t={412 + i*4 + 8}")
    ...
    print(f"New accumulator value: evaluate just after t={412 + i*4 + 12}")
    print(f"({accum_re:012x}, {accum_im:012x})")
    print(f"")
